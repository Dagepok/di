...
namespace FractalPainting.App.Fractals
{
	public class DragonPainter
	{
		...
		public DragonPainter(IImageHolder imageHolder, DragonSettings settings, Palette pallete)
		{
		...
		public void Paint()
		{
			...
			{
				graphics.FillRectangle(new SolidBrush(pallete.BackgroundColor), 0, 0, imageSize.Width, imageSize.Height);
				var r = new Random();
				...
				{
					graphics.FillRectangle(new SolidBrush(pallete.PrimaryColor), imageSize.Width/3f + p.X, imageSize.Height/2f + p.Y, 1, 1);
					p = r.Next(0, 2) == 0 
